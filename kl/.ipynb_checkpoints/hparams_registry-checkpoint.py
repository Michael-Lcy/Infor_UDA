...
def _hparams(algorithm, dataset, random_seed):
    ...
    if algorithm in ['KL', 'PERM', 'KL_GP']:
        _hparam('num_samples', 20, lambda r: 20)

        
    if algorithm == 'KL_GP':
        _hparam('grad_penalty', 0.1, lambda r: 10**r.uniform(-2, 1))
    ...
    if algorithm in ['KL', 'KL_GP']:
        _hparam('augment_softmax', 0.0, lambda r: r.choice([0.0,0.01,0.05]))
        if dataset == 'RotatedMNIST':
            _hparam('kl_reg', 0.2, lambda r: 10**r.uniform(-2, 0))
            _hparam('kl_reg_aux', 0.2, lambda r: 10**r.uniform(-2, 0))
            # OOM 预警：若二阶导数导致显存溢出，需将这里的 256 改为 128 或 64
