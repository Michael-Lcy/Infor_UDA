...
class KL_GP(KL):
    """
    KL with Gradient Penalty (Wang & Mao, 2023)
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(KL_GP, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.grad_penalty_weight = hparams['grad_penalty']
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.grad_penalty_weight > 0.0:
            ...
            # 对当前 obj 求一阶导数，必须设置 create_graph=True 保留计算图
            ...
            # 将梯度惩罚正则项并入总损失函数
            obj = obj + self.grad_penalty_weight * grad_norm_sq
            penalty_val = grad_norm_sq.item()
        # ==================================================
        ...
        return {'loss': loss.item(), 'kl': kl.item(), 'kl_aux': kl_aux.item(), 'penalty': penalty_val}
